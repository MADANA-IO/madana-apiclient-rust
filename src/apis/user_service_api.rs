...
pub trait UserServiceApi {
    ...
    fn get_enclave_history(&self, username: &str, limit: Option<&str>, offset: Option<&str>) -> Box<dyn Future<Item = std::path::PathBuf, Error = Error<serde_json::Value>>>;
...
impl<C: hyper::client::Connect>UserServiceApi for UserServiceApiClient<C> {
    ...
    fn get_enclave_history(&self, username: &str, limit: Option<&str>, offset: Option<&str>) -> Box<dyn Future<Item = std::path::PathBuf, Error = Error<serde_json::Value>>> {
        let mut req = __internal_request::Request::new(hyper::Method::Get, "/users/{username}/enclavehistory".to_string())
        ;

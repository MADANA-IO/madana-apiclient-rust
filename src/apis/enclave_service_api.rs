...
pub async fn get_stats(configuration: &configuration::Configuration, dayssince: Option<&str>) -> Result<std::path::PathBuf, Error<GetStatsError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/enclaves/stats", configuration.base_path);
    let mut local_var_req_builder = local_var_client.get(local_var_uri_str.as_str());

...
/// struct for typed errors of method `redist_test`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RedistTestError {
    UnknownValue(serde_json::Value),
}
...
pub async fn redist_test(configuration: &configuration::Configuration, ) -> Result<std::path::PathBuf, Error<RedistTestError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/enclaves/test/redis", configuration.base_path);
    let mut local_var_req_builder = local_var_client.get(local_var_uri_str.as_str());
    ...
    if local_var_status.is_success() {
        ...
        let local_var_entity: Option<RedistTestError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
